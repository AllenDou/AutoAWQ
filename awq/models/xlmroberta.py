...
from transformers.models.xlm_roberta.modeling_xlm_roberta import (
    XLMRobertaForSequenceClassification as OldXLMRobertaForSequenceClassification,
    XLMRobertaLayer as OldXLMRobertaLayer,
)
from awq.modules.fused.norm import FasterTransformerRMSNorm


#class Qwen2AWQForCausalLM(BaseAWQForCausalLM):
class XLMRobertaAWQForSequenceClassification(BaseAWQForCausalLM):
    #layer_type = "Qwen2DecoderLayer"
    #max_seq_len_key = "max_position_embeddings"
    layer_type = "XLMRobertaLayer"

    @staticmethod
    def fuse_layers(model: OldXLMRobertaForSequenceClassification):
        fuser = Qwen2Fuser(model)
        fuser.fuse_transformer()

    @staticmethod
    def get_model_layers(model: OldXLMRobertaForSequenceClassification):
        #return model.model.layers
        return model.roberta.encoder.layer

    @staticmethod
    def get_act_for_scaling(module: OldXLMRobertaLayer):
        return dict(is_scalable=False)

    @staticmethod
    def move_embed(model: OldXLMRobertaForSequenceClassification, device: str):
        #model.model.embed_tokens = model.model.embed_tokens.to(device)
        model.roberta.embeddings = model.roberta.embeddings.to(device)

    @staticmethod
    def get_layers_for_scaling(module: OldXLMRobertaLayer, input_feat, module_kwargs):
        ...
        #layers.append(
        #    dict(
        #        prev_op=module.attention.output.LayerNorm,
        #        layers=[
